-- Nexora Jobs: idempotent re-ensuring of authenticated location sync
-- (the `user_location` 404).
--
-- Live console: POST /rest/v1/rpc/sync_user_location answered 404 PGRST202 —
-- the function was never created on the deployed project because
-- 20260810090000_jobs_location_sync.sql had not been applied there.
-- src/services/locationSync.ts already degrades that to a clean 'unsupported'
-- state; this migration converges the live schema itself.
--
-- The entire body below is the ORIGINAL location-sync migration replayed with
-- its own idempotent guards (create if not exists / create or replace /
-- drop-if-exists policies and triggers / full grant re-assertion), so applying
-- it to a project that already has 20260810090000 is a complete no-op — and it
-- is safe for projects where that file was skipped, partially applied, or
-- half-rolled-back. Do not edit the definitions here; edit the original and
-- re-synchronize instead.
--
-- Body source of truth: supabase/migrations/20260810090000_jobs_location_sync.sql
