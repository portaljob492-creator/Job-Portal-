...
const fieldClass = 'w-full bg-[#f8fafc] text-[#0f172a] border border-[#cbd5e1]/70 rounded-xl px-3 py-3 focus:outline-none focus:ring-1 focus:ring-[#4f46e5] focus:border-[#4f46e5] text-xs font-semibold';
const labelClass = 'text-xs font-bold text-[#0f172a]';
...
export const SeekerProfileTab: React.FC<SeekerProfileTabProps> = ({
  ...
  if (!isEditing && confirmed) {
    return (
      <div className="max-w-3xl mx-auto py-4">
        <section className="bg-white rounded-3xl border border-[#cbd5e1]/60 shadow-[0_8px_30px_rgba(15,23,42,0.08)] overflow-hidden">
          <div className="bg-gradient-to-br from-[#4f46e5] to-[#7c3aed] px-6 py-10 text-white text-center">
          ...
          <div className="p-6 md:p-8 space-y-6">
            <div className="flex items-center gap-4 pb-6 border-b border-[#cbd5e1]/50">
              {userProfile.avatarUrl || avatarPreview ? (
                <img src={userProfile.avatarUrl || avatarPreview} alt={fullName} className="w-16 h-16 rounded-full object-cover border-2 border-[#ede9fe]" />
              ) : (
                <div className="w-16 h-16 rounded-full bg-[#ede9fe] text-[#4f46e5] flex items-center justify-center text-xl font-extrabold">{fullName.charAt(0)}</div>
              )}
              <div>
                <p className="text-[11px] uppercase tracking-wider text-[#64748b] font-bold">Candidate Name</p>
                <h2 className="text-xl font-extrabold text-[#0f172a]">{fullName}</h2>
                <p className="text-xs text-[#475569]">{headline}</p>
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div className="bg-[#f8fafc] rounded-2xl border border-[#cbd5e1]/50 p-4">
                <p className="text-[10px] uppercase tracking-wider text-[#64748b] font-bold">Profile Completion Status</p>
                <p className="text-lg font-extrabold text-[#4f46e5] mt-1">{confirmed.profileCompletion}% Complete</p>
              </div>
              <div className="bg-[#f8fafc] rounded-2xl border border-[#cbd5e1]/50 p-4">
                <p className="text-[10px] uppercase tracking-wider text-[#64748b] font-bold">Application Ready Status</p>
              ...
              <div className="bg-[#f8fafc] rounded-2xl border border-[#cbd5e1]/50 p-4 min-w-0">
                <p className="text-[10px] uppercase tracking-wider text-[#64748b] font-bold">Profile ID / Candidate ID</p>
                <p className="text-xs font-mono font-bold text-[#0f172a] mt-2 truncate" title={confirmed.candidateId}>{confirmed.candidateId}</p>
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <button onClick={() => onNavigateTab?.('feed')} className="rounded-full bg-[#7c3aed] text-white px-5 py-3 text-xs font-bold hover:bg-[#6d28d9] transition-colors">Search Jobs</button>
              <button onClick={() => onNavigateTab?.('applications')} className="rounded-full bg-[#4f46e5] text-white px-5 py-3 text-xs font-bold hover:bg-[#6d28d9] transition-colors">View My Applications</button>
              <button onClick={() => { setSubmission(null); setIsEditing(true); setStep(0); }} className="rounded-full bg-white border border-[#4f46e5] text-[#4f46e5] px-5 py-3 text-xs font-bold hover:bg-[#ede9fe]/20 transition-colors flex items-center justify-center gap-2"><Pencil className="w-4 h-4" /> Edit Profile</button>
            </div>
          </div>
        </section>
  ...
  if (!isEditing) {
    return (
      <div className="bg-white rounded-2xl border border-[#cbd5e1]/50 p-10 text-center">
        <User className="w-12 h-12 mx-auto text-[#4f46e5]" />
        <h2 className="text-xl font-extrabold mt-3">Complete your candidate profile</h2>
        <p className="text-sm text-[#475569] mt-1">Finish all eight steps before applying to employers.</p>
        <button onClick={() => setIsEditing(true)} className="mt-5 px-6 py-3 rounded-full bg-[#7c3aed] text-white text-xs font-bold">Start Profile</button>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto pb-28">
      <section className="bg-white rounded-2xl border border-[#cbd5e1]/50 shadow-sm overflow-hidden">
        <header className="px-5 md:px-7 pt-6 pb-4 border-b border-[#cbd5e1]/40">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-[11px] font-extrabold uppercase tracking-widest text-[#7c3aed]">Candidate Profile</p>
              <h1 className="text-xl md:text-2xl font-extrabold text-[#0f172a]">Complete your professional profile</h1>
              <p className="text-xs text-[#475569] mt-1">Step {step + 1} of 8 · Your details stay in this form if submission fails.</p>
            </div>
            <span className="shrink-0 rounded-full bg-[#ede9fe] text-[#4f46e5] px-3 py-1 text-xs font-bold">{Math.round(((step + 1) / 8) * 100)}%</span>
          </div>
          <div className="mt-5 flex gap-2 overflow-x-auto no-scrollbar pb-1" aria-label="Profile steps">
            {STEPS.map(({ label, icon: Icon }, index) => (
              <button
                ...
                className={`min-w-[112px] px-3 py-2 rounded-xl border text-left transition-colors ${index === step ? 'bg-[#4f46e5] border-[#4f46e5] text-white' : index < step ? 'bg-[#ede9fe]/30 border-[#cbd5e1] text-[#4f46e5]' : 'bg-[#f8fafc] border-[#cbd5e1]/60 text-[#64748b]'}`}
              >
          ...
          <div className="h-1 bg-[#f1f5f9] mt-4 rounded-full overflow-hidden"><div className="h-full bg-[#7c3aed] transition-all" style={{ width: `${((step + 1) / 8) * 100}%` }} /></div>
        </header>
        ...
        <div className="p-5 md:p-7 min-h-[410px]">
            ...
            <Step title="Personal" description="Tell employers who you are and what you do.">
              <div className="flex flex-col sm:flex-row gap-6 items-center sm:items-start">
                <div className="relative shrink-0">
                  {avatarPreview ? <img src={avatarPreview} alt="Profile preview" className="w-28 h-28 rounded-full object-cover border-4 border-[#ede9fe]" /> : <div className="w-28 h-28 rounded-full bg-[#ede9fe] text-[#4f46e5] grid place-items-center text-3xl font-bold">{fullName.charAt(0) || <User />}</div>}
                  <label className="absolute -bottom-1 -right-1 w-10 h-10 rounded-full bg-[#7c3aed] text-white grid place-items-center border-2 border-white cursor-pointer" title="Choose profile image">
          ...
          {step === 2 && (
            <Step title="Education" description="Add beauty school, degree or professional training. This step is optional.">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 bg-[#f8fafc] p-4 rounded-2xl border border-[#cbd5e1]/40">
                ...
                <button type="button" onClick={addEducation} className="sm:col-span-3 justify-self-start px-4 py-2 rounded-full bg-[#4f46e5] text-white text-xs font-bold flex items-center gap-1"><Plus className="w-4 h-4" /> Add Education</button>
              </div>
          ...
          {step === 3 && (
            <Step title="Experience" description="Add your experience level and relevant salon work.">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 bg-[#f8fafc] p-4 rounded-2xl border border-[#cbd5e1]/40">
                ...
                <label className="sm:col-span-2 flex items-center gap-2 text-xs font-semibold text-[#475569]"><input type="checkbox" checked={experienceDraft.currentlyWorking} onChange={(e) => setExperienceDraft({ ...experienceDraft, currentlyWorking: e.target.checked, endDate: '' })} /> I currently work here</label>
                ...
                <button type="button" onClick={addExperience} className="sm:col-span-2 justify-self-start px-4 py-2 rounded-full bg-[#4f46e5] text-white text-xs font-bold flex items-center gap-1"><Plus className="w-4 h-4" /> Add Experience</button>
              </div>
          ...
          {step === 4 && (
            <Step title="Skills" description="Add at least one skill employers can match with their jobs.">
              <div className="flex gap-2"><input className={fieldClass} value={skillDraft} onChange={(e) => setSkillDraft(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); addSkill(); } }} placeholder="e.g. Balayage, Nail Art, Facials" /><button type="button" onClick={addSkill} className="px-5 rounded-xl bg-[#4f46e5] text-white text-xs font-bold">Add</button></div>
              <div className="flex flex-wrap gap-2">{skills.map((skill) => <span key={skill} className="px-3 py-2 rounded-full bg-[#ede9fe] text-[#4f46e5] text-xs font-bold flex items-center gap-2">{skill}<button type="button" onClick={() => setSkills((current) => current.filter((item) => item !== skill))}><Trash2 className="w-3 h-3" /></button></span>)}</div>
            </Step>
          )}

          {step === 5 && (
            <Step title="Preferred Job" description="Set the roles, work types and compensation you prefer.">
              <div className="flex gap-2"><input className={fieldClass} value={roleDraft} onChange={(e) => setRoleDraft(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); addRole(); } }} placeholder="e.g. Senior Colorist" /><button type="button" onClick={addRole} className="px-5 rounded-xl bg-[#4f46e5] text-white text-xs font-bold">Add Role</button></div>
              <div className="flex flex-wrap gap-2">{preferredRoles.map((role) => <span key={role} className="px-3 py-2 rounded-full bg-[#ede9fe] text-[#4f46e5] text-xs font-bold flex items-center gap-2">{role}<button type="button" onClick={() => setPreferredRoles((current) => current.filter((item) => item !== role))}><Trash2 className="w-3 h-3" /></button></span>)}</div>
              <div><p className={`${labelClass} mb-2`}>Employment Types *</p><div className="flex flex-wrap gap-2">{EMPLOYMENT_TYPES.map((type) => <label key={type.value} className={`px-3 py-2 rounded-full border text-xs font-bold cursor-pointer ${employmentTypes.includes(type.value) ? 'bg-[#4f46e5] border-[#4f46e5] text-white' : 'bg-white border-[#cbd5e1] text-[#475569]'}`}><input type="checkbox" className="sr-only" checked={employmentTypes.includes(type.value)} onChange={() => setEmploymentTypes((current) => current.includes(type.value) ? current.filter((item) => item !== type.value) : [...current, type.value])} />{type.label}</label>)}</div></div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <label className="flex items-center gap-3 rounded-xl bg-[#f8fafc] border border-[#cbd5e1]/60 p-3 text-xs font-bold"><input type="checkbox" checked={openToRelocation} onChange={(e) => setOpenToRelocation(e.target.checked)} /> Open to relocation</label>
              </div>
            </Step>
          )}
          ...
          {step === 6 && (
            <Step title="Documents" description="Upload a resume and add certifications if available. Files upload securely when you submit.">
              <label className="border-2 border-dashed border-[#cbd5e1] rounded-2xl p-6 text-center cursor-pointer hover:border-[#4f46e5] block">
                <Upload className="w-8 h-8 text-[#4f46e5] mx-auto" /><p className="text-xs font-bold mt-2">{pendingResume ? pendingResume.name : 'Choose PDF, DOC or DOCX resume (max 5MB)'}</p>
              ...
              {resumesLoading ? <JobsInlineSkeleton rows={2} label="Loading resumes" /> : resumes.map((resume) => <div key={resume.id} className="flex items-center gap-3 rounded-xl border border-[#cbd5e1]/60 p-3"><FileText className="w-5 h-5 text-[#4f46e5]" /><div className="min-w-0 flex-1"><p className="text-xs font-bold truncate">{resume.fileName}</p><button type="button" onClick={() => void makePrimary(resume)} disabled={resume.isPrimary} className="text-[10px] text-[#4f46e5] font-bold disabled:text-emerald-700">{resume.isPrimary ? 'Primary resume' : 'Make primary'}</button></div><button type="button" onClick={() => void openResume(resume)}><Download className="w-4 h-4 text-[#475569]" /></button><button type="button" onClick={() => void removeResume(resume)}><Trash2 className="w-4 h-4 text-rose-600" /></button></div>)}
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 bg-[#f8fafc] p-4 rounded-2xl border border-[#cbd5e1]/40">
                ...
                <button type="button" onClick={addCertification} className="sm:col-span-3 justify-self-start px-4 py-2 rounded-full bg-[#4f46e5] text-white text-xs font-bold flex items-center gap-1"><Plus className="w-4 h-4" /> Add Certification</button>
              </div>
      ...
      <div className="fixed bottom-20 sm:bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-[#cbd5e1]/50 p-4 shadow-[0_-4px_12px_rgba(15,23,42,0.06)]">
        <div className="max-w-4xl mx-auto flex justify-between gap-3">
          <button type="button" onClick={() => { if (step > 0) setStep((current) => current - 1); else if (confirmed) setIsEditing(false); }} disabled={step === 0 && !confirmed} className="px-5 py-3 rounded-full border border-[#64748b] text-[#475569] text-xs font-bold disabled:opacity-40 flex items-center gap-2"><ArrowLeft className="w-4 h-4" /> Back</button>
          {step < 7 ? <button type="button" onClick={goNext} className="px-7 py-3 rounded-full bg-[#7c3aed] text-white text-xs font-bold flex items-center gap-2">Continue <ArrowRight className="w-4 h-4" /></button> : <button type="button" onClick={() => void handleSubmit()} disabled={isSubmitting} className="min-w-[220px] px-7 py-3 rounded-full bg-[#7c3aed] disabled:opacity-70 text-white text-xs font-bold flex items-center justify-center gap-2">{isSubmitting ? <><LoaderCircle className="w-4 h-4 animate-spin" /> Submitting your profile...</> : <><CheckCircle2 className="w-4 h-4" /> Submit Profile</>}</button>}
        </div>
      </div>
      {toastMessage && <Toast message={toastMessage} />}
    </div>
...
const Step: React.FC<{ title: string; description: string; children: React.ReactNode }> = ({ title, description, children }) => <div className="space-y-5 animate-in fade-in duration-200"><div><h2 className="text-lg font-extrabold text-[#0f172a]">{title}</h2><p className="text-xs text-[#475569] mt-1">{description}</p></div>{children}</div>;
...
const Toast: React.FC<{ message: string }> = ({ message }) => <div role="status" className="fixed bottom-24 right-6 z-[80] bg-[#0f172a] text-white px-4 py-3 rounded-2xl shadow-xl border border-white/20 flex items-center gap-2 text-xs font-bold"><CheckCircle2 className="w-4 h-4 text-emerald-400" />{message}</div>;
const RecordList: React.FC<{ empty: string; items: Array<{ id: string; title: string; subtitle: string }>; onRemove: (id: string) => void }> = ({ empty, items, onRemove }) => items.length === 0 ? <p className="rounded-xl border border-dashed border-[#cbd5e1] p-4 text-center text-xs text-[#64748b]">{empty}</p> : <div className="space-y-2">{items.map((item) => <div key={item.id} className="rounded-xl border border-[#cbd5e1]/60 p-3 flex items-start gap-3"><div className="flex-1 min-w-0"><p className="text-xs font-bold text-[#0f172a]">{item.title}</p><p className="text-[11px] text-[#475569] mt-0.5">{item.subtitle}</p></div><button type="button" onClick={() => onRemove(item.id)} className="text-rose-600 p-1"><Trash2 className="w-4 h-4" /></button></div>)}</div>;
const Summary: React.FC<{ title: string; value: string; onEdit: () => void }> = ({ title, value, onEdit }) => <div className="rounded-2xl bg-[#f8fafc] border border-[#cbd5e1]/60 p-4"><div className="flex justify-between gap-2"><p className="text-xs font-extrabold text-[#4f46e5]">{title}</p><button type="button" onClick={onEdit} className="text-[10px] font-bold text-[#6d28d9]">Edit</button></div><p className="text-[11px] text-[#475569] mt-1 break-words">{value}</p></div>;
