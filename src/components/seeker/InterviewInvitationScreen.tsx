...
export const InterviewInvitationScreen: React.FC<InterviewInvitationScreenProps> = ({
    ...
    return (
      <div className="min-h-screen bg-[#f8fafc] text-[#0f172a]">
        <header className="sticky top-0 z-40 flex h-16 items-center border-b border-[#cbd5e1]/30 bg-white px-5 shadow-xs">
          ...
          <h1 className="mx-auto pr-10 text-lg font-bold text-[#4f46e5]">Interview Invitation</h1>
        </header>
        <main className="mx-auto max-w-xl px-5 py-16 text-center">
          <Calendar className="mx-auto mb-4 h-10 w-10 text-[#64748b]" />
          <h2 className="text-lg font-bold">No interview invitation</h2>
          <p className="mt-2 text-sm text-[#475569]">When an employer schedules an interview, its confirmed details will appear here.</p>
  ...
  return (
    <div id="interview_invitation_screen" className="relative min-h-screen bg-[#f8fafc] text-[#0f172a] pb-40">
      {/* Toast Alert */}
      {toastMessage && (
      ...
      {/* Top Header App Bar */}
      <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-[#cbd5e1]/30 shadow-xs">
        <div className="max-w-3xl mx-auto flex justify-between items-center px-margin-side h-16 w-full">
          <button
            id="back_to_workspace"
            onClick={onBack}
            className="w-10 h-10 flex items-center justify-center rounded-full text-[#475569] hover:bg-[#f1f5f9] transition-all active:scale-95 duration-200"
          >
            <ArrowLeft className="w-5 h-5" />
          </button>

          <h1 className="text-lg font-bold text-[#4f46e5] tracking-tight">Nexora Jobs</h1>
      ...
      <main className="max-w-3xl mx-auto pt-6 px-margin-side animate-in fade-in duration-500">
        ...
        <div className="mb-section-gap">
          <h2 className="text-2xl font-extrabold text-[#4f46e5] tracking-tight leading-tight">Interview Invitation</h2>
          <p className="text-[#475569] text-xs font-semibold mt-1">
        ...
        {/* Job Context Card */}
        <div className="bg-white border border-[#cbd5e1]/50 rounded-2xl p-stack-default mb-section-gap shadow-sm">
          <div className="flex items-center gap-stack-default">
            <div className="w-16 h-16 rounded-xl overflow-hidden border border-[#cbd5e1]/30 bg-[#f8fafc] shrink-0 flex items-center justify-center">
              {activeApp.salonLogo ? (
                <img
            ...
            <div>
              <h3 className="text-lg font-bold text-[#0f172a] leading-snug">{activeApp.jobTitle}</h3>
              <p className="text-xs font-bold text-[#7c3aed]">{activeApp.salonName}</p>
              <p className="text-[11px] text-[#475569] mt-0.5">{activeApp.location}</p>
            </div>
          </div>
        </div>
        ...
        <div className="mb-section-gap">
          <h3 className="mb-stack-default text-base font-bold text-[#0f172a]">Interview Details</h3>
          <div className="grid grid-cols-1 gap-gutter sm:grid-cols-2">
            <div className="flex items-start gap-stack-default rounded-2xl border border-[#cbd5e1]/40 bg-white p-stack-default shadow-xs">
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#ede9fe] text-[#4f46e5]">
                <Calendar className="h-5 w-5" />
              </div>
              <div>
                <p className="mb-0.5 text-[10px] font-bold uppercase tracking-wider text-[#475569]">Date & Time</p>
                <p className="text-sm font-bold text-[#0f172a]">{interviewTimeText}</p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-stack-default rounded-2xl border border-[#cbd5e1]/40 bg-white p-stack-default shadow-xs">
              ...
              <div>
                <p className="mb-0.5 text-[10px] font-bold uppercase tracking-wider text-[#475569]">Duration</p>
                <p className="text-sm font-bold text-[#0f172a]">{activeApp.interviewDurationMinutes ? `${activeApp.interviewDurationMinutes} minutes` : 'To be confirmed'}</p>
              </div>
            </div>

            <div className="flex items-start gap-stack-default rounded-2xl border border-[#cbd5e1]/40 bg-white p-stack-default shadow-xs sm:col-span-2">
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#ede9fe] text-[#4f46e5]">
              ...
              <div className="min-w-0 flex-grow">
                <p className="mb-0.5 text-[10px] font-bold uppercase tracking-wider text-[#475569]">Interview Type</p>
                <p className="text-sm font-bold text-[#0f172a]">{interviewTypeLabel}</p>
                {activeApp.interviewLocation && (
                  <p className="mt-1 text-xs font-semibold text-[#475569]">{activeApp.interviewLocation}</p>
                )}
                {activeApp.interviewMeetingUrl && (
                  <div className="mt-2 flex items-center justify-between gap-3 rounded-xl border border-[#cbd5e1]/40 bg-[#f8fafc] px-3 py-2">
                    <a href={activeApp.interviewMeetingUrl} target="_blank" rel="noreferrer" className="truncate text-xs font-medium text-[#4f46e5] hover:underline">
                      {activeApp.interviewMeetingUrl}
                    </a>
                    <button type="button" onClick={() => void handleCopyLink()} className="flex shrink-0 items-center gap-1 text-xs font-bold text-[#4f46e5] hover:underline">
        ...
        <div className="mb-section-gap">
          <h3 className="text-base font-bold text-[#0f172a] mb-stack-default">Message from Employer</h3>
          <div className="bg-[#ede9fe] rounded-2xl p-stack-default border border-[#cbd5e1]/50 relative overflow-hidden shadow-xs">
            {/* Elegant decorative background accent */}
            <div className="absolute top-0 right-0 w-16 h-16 bg-gradient-to-br from-[#4f46e5]/5 to-transparent rounded-bl-full pointer-events-none" />

            <p className="text-sm text-[#0f172a] italic font-medium leading-relaxed pl-2 border-l-2 border-[#4f46e5]">
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-[#cbd5e1]/40 py-4 px-margin-side shadow-lg z-30 pb-20 md:pb-4">
        <div className="mx-auto max-w-3xl">
          ...
          <button
            ...
            className="w-full sm:w-auto font-bold text-xs text-[#4f46e5] border border-[#4f46e5] px-6 py-3 rounded-full hover:bg-[#ede9fe] transition-colors order-2 cursor-pointer text-center disabled:cursor-not-allowed disabled:opacity-50"
          >
          ...
          <button
            ...
            className="w-full sm:w-auto font-bold text-xs bg-[#4f46e5] text-white px-8 py-3 rounded-full hover:bg-[#6d28d9] transition-all duration-200 shadow-sm hover:shadow-md order-1 sm:order-3 cursor-pointer text-center disabled:bg-emerald-600 disabled:hover:bg-emerald-600"
          >
      ...
      {showRescheduleModal && (
        <div className="fixed inset-0 z-100 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 border border-[#cbd5e1] shadow-2xl animate-scale-up">
            <div className="flex justify-between items-center pb-3 border-b border-[#cbd5e1]/30 mb-4">
              <div>
                <h3 className="text-base font-extrabold text-[#0f172a]">Propose New Time</h3>
                <p className="text-[10px] text-[#475569]">Request a schedule change from the salon coordinator</p>
              </div>
              <button
                onClick={() => setShowRescheduleModal(false)}
                className="w-8 h-8 rounded-full hover:bg-[#f1f5f9] flex items-center justify-center text-[#475569] cursor-pointer"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <form onSubmit={handleRescheduleSubmit} className="space-y-4 text-xs">
              <div>
                <label className="font-semibold text-[#0f172a] block mb-1">Preferred Date</label>
                <input
                  type="date"
                  required
                  value={rescheduleDate}
                  onChange={(e) => setRescheduleDate(e.target.value)}
                  className="w-full bg-[#f8fafc] border border-[#cbd5e1] rounded-xl p-3 text-xs text-[#0f172a] outline-none focus:ring-2 focus:ring-[#4f46e5]"
                />
              </div>

              <div>
                <label className="font-semibold text-[#0f172a] block mb-1">Preferred Time</label>
                <input
                  type="time"
                  required
              ...
              <div>
                <label className="font-semibold text-[#0f172a] block mb-1">Reason for Rescheduling (Optional)</label>
                <textarea
                  rows={2}
                  placeholder="e.g. Schedule conflict with model test, prior client booking, etc."
                  value={rescheduleReason}
                  onChange={(e) => setRescheduleReason(e.target.value)}
                  className="w-full bg-[#f8fafc] border border-[#cbd5e1] rounded-xl p-3 text-xs text-[#0f172a] outline-none focus:ring-2 focus:ring-[#4f46e5] placeholder-[#475569]/60"
                />
              </div>
              ...
              <div className="pt-2 flex gap-3">
                <button
                  type="button"
                  onClick={() => setShowRescheduleModal(false)}
                  className="flex-1 py-2.5 rounded-full text-xs font-bold text-[#475569] bg-[#f1f5f9]"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={Boolean(pendingAction)}
                  className="flex-1 py-2.5 rounded-full text-xs font-bold bg-[#4f46e5] text-white hover:bg-[#6d28d9] disabled:cursor-not-allowed disabled:opacity-50"
                >
      ...
      <nav className="fixed bottom-0 left-0 right-0 w-full flex justify-around items-center px-2 py-3 bg-white border-t border-[#cbd5e1]/40 rounded-t-2xl z-40 shadow-lg md:hidden">
        <button
          onClick={() => handleBottomNavClick('explore')}
          className="flex flex-col items-center justify-center text-[#475569] hover:text-[#4f46e5] active:scale-90 transition-transform w-14 cursor-pointer"
        >
        ...
        <button
          ...
          className="flex flex-col items-center justify-center bg-[#ede9fe] text-[#4f46e5] rounded-2xl px-4 py-1.5 active:scale-90 transition-transform cursor-pointer"
        >
