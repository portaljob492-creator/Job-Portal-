...
export const SupportScreen: React.FC<SupportScreenProps> = ({ onBack, userEmail, onNavigateTab }) => {
  ...
  const faqCategories: Record<string, { title: string; subtitle: string; icon: React.ReactNode; colorClass: string; items: { q: string; a: string }[] }> = {
    'Help Center': {
      ...
      colorClass: 'bg-[#ede9fe] text-[#4f46e5]',
      items: [
    ...
    'Application Help': {
      ...
      colorClass: 'bg-[#ede9fe] text-[#4338ca]',
      items: [
    ...
    'Account Help': {
      ...
      items: [
        ...
        { q: 'Can I use the same email for Seeker and Employer access?', a: 'No. Each email is permanently linked to one portal type, so use a separate email address to create an Employer account. Signing in on the wrong tab is fine though — we detect your account type and open the right portal for you.' }
      ]
    },
    'Resume Help': {
      title: 'Resume Help',
      ...
      colorClass: 'bg-[#f1f5f9] text-[#475569]',
      items: [
  ...
  if (subView === 'contact_form') {
    return (
      <div className="min-h-screen bg-[#f8fafc] text-[#0f172a] pb-24 relative select-none font-body-md">
        {/* TopAppBar exact replication of the HTML mock style */}
        <header className="sticky top-0 bg-white border-b border-[#cbd5e1]/30 px-margin-side h-16 w-full flex justify-between items-center z-40 shadow-[0_4px_12px_rgba(15,23,42,0.05)]">
          <button
          ...
          <h1 className="font-screen-title text-base font-extrabold text-[#4f46e5] tracking-tight absolute left-1/2 -translate-x-1/2">
            Contact Support
          </h1>
        ...
        <main className="max-w-2xl mx-auto px-margin-side py-8 space-y-6">
          <p className="text-xs text-[#475569] text-center max-w-md mx-auto leading-relaxed">
          ...
          <form onSubmit={handleFormSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              <label className="block text-xs font-bold text-[#0f172a]">What can we help you with?</label>
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-full border text-xs font-semibold transition-all cursor-pointer ${
                      issueType === item.value
                        ? 'bg-[#ede9fe] text-[#4f46e5] border-[#4f46e5] shadow-sm'
            ...
            <div className="space-y-1.5">
              <label className="block text-xs font-bold text-[#0f172a]" htmlFor="subject">Subject</label>
              <input
                id="subject"
                type="text"
                required
                ...
                className="w-full bg-[#f1f5f9] border-transparent focus:border-[#4f46e5] focus:bg-white focus:ring-1 focus:ring-[#4f46e5] rounded-lg py-3 px-4 text-xs font-semibold text-[#0f172a] placeholder:text-[#64748b]/60 transition-colors outline-none"
              />
            </div>

            {/* Description */}
            <div className="space-y-1.5">
              <label className="block text-xs font-bold text-[#0f172a]" htmlFor="description">Description</label>
              <textarea
                id="description"
                required
                rows={5}
                ...
                className="w-full bg-[#f1f5f9] border-transparent focus:border-[#4f46e5] focus:bg-white focus:ring-1 focus:ring-[#4f46e5] rounded-lg py-3 px-4 text-xs font-semibold text-[#0f172a] placeholder:text-[#64748b]/60 transition-colors custom-scrollbar resize-y outline-none"
              />
            </div>

            {/* Screenshot Upload with drag and drop */}
            <div className="space-y-1.5">
              <label className="block text-xs font-bold text-[#0f172a]">Attachments</label>
              ...
              <div
                ...
                className={`border-2 border-dashed rounded-lg p-6 flex flex-col items-center justify-center text-center cursor-pointer transition-all duration-200 group ${
                  isDragging
                    ? 'border-[#4f46e5] bg-[#ede9fe]/15'
                    : 'border-[#cbd5e1] bg-[#f1f5f9]/40 hover:bg-[#f1f5f9]/70'
                }`}
              >
                <input
                  ref={fileInputRef}
                  ...
                  className="hidden"
                />
                <span className="material-symbols-outlined text-4xl text-[#64748b] group-hover:text-[#4f46e5] transition-colors mb-2">
                  cloud_upload
                </span>
                <span className="text-xs font-bold text-[#475569] group-hover:text-[#0f172a] transition-colors">
              ...
              {uploadedFiles.length > 0 && (
                <div className="mt-3 space-y-2">
                  {uploadedFiles.map((file, idx) => (
                    <div key={idx} className="flex items-center justify-between bg-white p-3 rounded-lg border border-[#cbd5e1]/30 text-xs shadow-xs">
                      <div className="flex items-center gap-2">
                        <FileText className="w-4 h-4 text-[#4f46e5]" />
                        <div>
                          <p className="font-bold text-[#0f172a] truncate max-w-[200px]">{file.name}</p>
            ...
            <div className="pt-2 space-y-2">
              ...
              <button
                type="submit"
                disabled={ticketBusy}
                className="w-full bg-[#6d28d9] text-white font-bold text-xs rounded-full py-4 px-6 hover:bg-[#4f46e5] disabled:opacity-60 transition-all active:scale-95 shadow-[0_4px_12px_rgba(109,40,217,0.2)] flex justify-center items-center gap-2 cursor-pointer"
              >
        ...
        {toastMessage && (
          <div className="fixed bottom-24 left-1/2 -translate-x-1/2 z-50 bg-[#0f172a]/95 text-white px-5 py-4 rounded-2xl shadow-2xl border border-white/10 flex items-center gap-2.5 text-xs font-bold animate-bounce text-center whitespace-nowrap">
  ...
  if (subView === 'contact_success') {
    return (
      <div className="min-h-screen bg-[#f8fafc] text-[#0f172a] pb-24 relative select-none font-body-md flex flex-col justify-between">
        {/* TopAppBar */}
        <header className="sticky top-0 bg-white border-b border-[#cbd5e1]/30 px-margin-side h-16 w-full flex justify-between items-center z-40 shadow-[0_4px_12px_rgba(15,23,42,0.05)]">
          <button
            onClick={() => setSubView('home')}
        ...
        <main className="flex-1 max-w-md mx-auto w-full flex flex-col items-center justify-center p-6 min-h-[60vh]">
          <div className="bg-white rounded-2xl border border-[#cbd5e1]/30 shadow-[0_8px_24px_rgba(15,23,42,0.08)] p-8 flex flex-col items-center text-center w-full">
            <div className="w-20 h-20 rounded-full bg-[#ede9fe] flex items-center justify-center mb-6 shadow-xs">
              <CheckCircle className="w-10 h-10 text-[#4f46e5]" />
            </div>

            <h2 className="text-sm font-bold text-[#0f172a] mb-2">Support request submitted</h2>
            <p className="text-[11px] text-[#475569] mb-6 leading-relaxed">
              Thank you for reaching out. Our beauty and wellness support team will review your ticket and get back to you shortly.
            </p>

            <div className="bg-[#f1f5f9] py-3 px-8 rounded-xl mb-8 border border-[#cbd5e1]/20">
              <span className="text-[10px] font-bold text-[#64748b] uppercase tracking-widest block">Ticket ID</span>
              <p className="text-sm font-extrabold text-[#4f46e5] mt-1">{submittedTicketId}</p>
            </div>

            <button
              onClick={handleBackToSupport}
              className="w-full bg-[#f1f5f9] text-[#0f172a] font-bold text-xs rounded-full py-4 px-6 hover:bg-[#cbd5e1]/50 transition-colors border border-[#cbd5e1]/50 active:scale-95 cursor-pointer"
            >
              Back to Support
            </button>
          </div>
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-[#0f172a] pb-24 relative select-none font-body-md">
      ...
      <header className="sticky top-0 bg-white border-b border-[#cbd5e1]/30 px-margin-side h-16 w-full flex justify-between items-center z-40 shadow-[0_4px_12px_rgba(15,23,42,0.05)]">
        ...
        <div className="text-center">
          <div className="flex items-center gap-1 justify-center">
            ...
            <h1 className="font-screen-title text-base font-extrabold text-[#4f46e5] tracking-tight">Nexora Jobs</h1>
          </div>
          <span className="text-[9px] font-bold text-[#64748b] uppercase tracking-wider block bg-[#ede9fe]/30 px-2 py-0.5 rounded-md mt-0.5 font-mono">
            /app/jobs/support
          </span>
        </div>
        <button
          onClick={() => triggerToast("🔔 Checking for new beauty workspace alerts...")}
          className="w-10 h-10 flex items-center justify-center rounded-full hover:bg-[#ede9fe]/50 text-[#64748b] transition-colors active:scale-95 duration-200 cursor-pointer"
      ...
      <main className="max-w-4xl mx-auto px-margin-side py-6 space-y-6">
        ...
        <section className="bg-gradient-to-r from-[#4f46e5] via-[#6d28d9] to-[#7c3aed] text-white rounded-2xl p-6 shadow-[0_4px_12px_rgba(15,23,42,0.05)] border border-white/10 relative overflow-hidden">
        ...
        <section className="relative">
          <div className="relative group">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-[#64748b] group-focus-within:text-[#4f46e5] transition-colors w-5 h-5" />
            <input
              type="text"
              placeholder="Search help articles..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full bg-[#f1f5f9] py-3.5 pl-12 pr-12 rounded-lg border border-transparent focus:bg-white focus:border-[#4f46e5] focus:ring-1 focus:ring-[#4f46e5] outline-none transition-all duration-200 text-sm placeholder:text-[#64748b] shadow-sm"
            />
            {searchQuery && (
              <button
                onClick={() => setSearchQuery('')}
                className="absolute right-4 top-1/2 -translate-y-1/2 text-xs font-bold text-[#64748b] hover:text-[#4f46e5] bg-white/60 p-1 rounded-md"
              >
                Clear
              </button>
            )}
        ...
        {searchQuery.trim().length > 1 && (
          <section className="bg-white p-5 rounded-2xl border border-[#cbd5e1] shadow-sm space-y-3.5 animate-in fade-in duration-200">
            <h3 className="text-xs font-extrabold text-[#4f46e5] uppercase tracking-wider flex items-center gap-1.5">
              <Search className="w-3.5 h-3.5" />
              Found {matchedFaqItems.length} matching questions & answers
            </h3>

            {matchedFaqItems.length > 0 ? (
              <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2 divide-y divide-[#cbd5e1]/30">
                {matchedFaqItems.map((item, idx) => (
                  <div key={idx} className="pt-2.5 first:pt-0 space-y-1">
                    <span className="text-[10px] font-extrabold text-[#7c3aed] bg-[#ede9fe]/30 px-2 py-0.5 rounded-md uppercase tracking-wide">
                      {item.category}
                    </span>
                    <h4 className="text-xs font-extrabold text-[#0f172a]">{item.q}</h4>
                    <p className="text-xs text-[#475569] leading-relaxed bg-[#f8fafc] p-2.5 rounded-lg border border-[#cbd5e1]/20">
                      {item.a}
                    </p>
                  </div>
                ))}
        ...
        <section className="space-y-3">
          <h3 className="text-xs font-extrabold text-[#475569] uppercase tracking-wider">Help Directories & Trust Services</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <button
              ...
              className="bg-white border border-[#cbd5e1]/50 rounded-xl p-4 shadow-[0_4px_12px_rgba(15,23,42,0.05)] hover:shadow-md hover:bg-[#f1f5f9]/50 transition-all duration-200 flex items-start gap-4 text-left group cursor-pointer"
            >
              <div className="w-10 h-10 rounded-full bg-[#ede9fe] text-[#4f46e5] flex items-center justify-center shrink-0">
                <BookOpen className="w-5 h-5" />
              </div>
              <div>
                <h4 className="font-semibold text-xs text-[#0f172a] group-hover:text-[#4f46e5] transition-colors mb-1">Help Center</h4>
                <p className="text-[11px] text-[#475569] leading-tight">Browse all articles and FAQs.</p>
              </div>
            </button>
            ...
            >
              ...
              <div>
                <h4 className="font-semibold text-xs text-[#0f172a] group-hover:text-[#4f46e5] transition-colors mb-1">Application Help</h4>
                <p className="text-[11px] text-[#475569] leading-tight">Track status and manage apps.</p>
              </div>
            </button>
            ...
            >
              ...
              <div>
                <h4 className="font-semibold text-xs text-[#0f172a] group-hover:text-[#4f46e5] transition-colors mb-1">Account Help</h4>
                <p className="text-[11px] text-[#475569] leading-tight">Settings, login, and profile info.</p>
              </div>
            </button>
            ...
            >
              ...
              <div>
                <h4 className="font-semibold text-xs text-[#0f172a] group-hover:text-[#4f46e5] transition-colors mb-1">Resume Help</h4>
                <p className="text-[11px] text-[#475569] leading-tight">Uploads, formatting, and tips.</p>
              </div>
            </button>
            ...
            >
              ...
              <div>
                <h4 className="font-semibold text-xs text-[#0f172a] group-hover:text-[#4f46e5] transition-colors mb-1">Employer Issues</h4>
                <p className="text-[11px] text-[#475569] leading-tight">Interactions and company info.</p>
              </div>
            </button>
            ...
            >
              ...
              <div>
                <h4 className="font-semibold text-xs text-[#0f172a] group-hover:text-[#4f46e5] transition-colors mb-1">Safety</h4>
                <p className="text-[11px] text-[#475569] leading-tight">Guidelines and secure practices.</p>
              </div>
            </button>
            ...
            <button
              ...
              className="bg-white border border-[#cbd5e1]/50 rounded-xl p-4 shadow-[0_4px_12px_rgba(15,23,42,0.05)] hover:shadow-md hover:bg-amber-50/40 transition-all duration-200 flex items-start gap-4 text-left group cursor-pointer"
            >
              <div className="w-10 h-10 rounded-full bg-amber-100 text-amber-700 flex items-center justify-center shrink-0">
                <Flag className="w-5 h-5" />
              </div>
              <div>
                <h4 className="font-semibold text-xs text-[#0f172a] group-hover:text-amber-700 transition-colors mb-1">Report Job</h4>
                <p className="text-[11px] text-[#475569] leading-tight">Flag suspicious listings.</p>
              </div>
            </button>
            ...
            <button
              ...
              className="bg-white border border-[#cbd5e1]/50 rounded-xl p-4 shadow-[0_4px_12px_rgba(15,23,42,0.05)] hover:shadow-md hover:bg-rose-50/40 transition-all duration-200 flex items-start gap-4 text-left group cursor-pointer"
            >
              <div className="w-10 h-10 rounded-full bg-rose-100 text-rose-700 flex items-center justify-center shrink-0">
                <AlertOctagon className="w-5 h-5" />
              </div>
              <div>
                <h4 className="font-semibold text-xs text-[#0f172a] group-hover:text-rose-700 transition-colors mb-1">Report Employer</h4>
                <p className="text-[11px] text-[#475569] leading-tight">Report inappropriate conduct.</p>
              </div>
            </button>
            ...
            <button
              ...
              className="bg-white border border-[#cbd5e1]/50 rounded-xl p-4 shadow-[0_4px_12px_rgba(15,23,42,0.05)] hover:shadow-md hover:bg-[#f1f5f9]/50 transition-all duration-200 flex items-start gap-4 text-left group cursor-pointer"
            >
              <div className="w-10 h-10 rounded-full bg-[#f1f5f9] text-[#475569] flex items-center justify-center shrink-0">
                <Lock className="w-5 h-5" />
              </div>
              <div>
                <h4 className="font-semibold text-xs text-[#0f172a] group-hover:text-[#4f46e5] transition-colors mb-1">Privacy & Safety</h4>
                <p className="text-[11px] text-[#475569] leading-tight">Data policies and security.</p>
              </div>
            </button>
        ...
        {activeFaqCategory && faqCategories[activeFaqCategory] && (
          <section className="bg-white p-5 rounded-2xl border border-[#cbd5e1] shadow-sm space-y-4 animate-in slide-in-from-bottom duration-300">
            <div className="flex items-center justify-between pb-3 border-b border-[#cbd5e1]/30">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="font-extrabold text-sm text-[#0f172a]">{faqCategories[activeFaqCategory].title} Guides</h3>
                  <p className="text-[10px] text-[#64748b]">{faqCategories[activeFaqCategory].subtitle}</p>
                </div>
              </div>
              <button
                onClick={() => setActiveFaqCategory(null)}
                className="text-xs font-extrabold text-[#64748b] hover:text-[#4f46e5] hover:bg-[#ede9fe]/30 px-3 py-1 rounded-full transition-all cursor-pointer"
              >
                Close Folder
              </button>
            </div>
            ...
            <div className="space-y-2">
              {faqCategories[activeFaqCategory].items.map((item, index) => (
                <div key={index} className="border border-[#cbd5e1]/30 rounded-xl overflow-hidden bg-[#f8fafc]">
                  <button
                    onClick={() => setExpandedFaq(expandedFaq === index ? null : index)}
                    className="w-full p-3.5 hover:bg-[#ede9fe]/10 text-left text-xs font-extrabold text-[#0f172a] flex items-center justify-between gap-3"
                  >
                    <span>{item.q}</span>
                    {expandedFaq === index ? <ChevronUp className="w-4 h-4 text-[#4f46e5]" /> : <ChevronDown className="w-4 h-4 text-[#64748b]" />}
                  </button>
                  {expandedFaq === index && (
                    <div className="p-4 bg-white border-t border-[#cbd5e1]/20 text-xs text-[#475569] leading-relaxed">
                      {item.a}
                    </div>
                  )}
                </div>
        ...
        <section className="bg-white border border-[#cbd5e1]/60 rounded-xl p-8 shadow-[0_4px_12px_rgba(15,23,42,0.05)] text-center relative overflow-hidden">
          <div className="absolute top-0 right-0 w-32 h-32 bg-[#4f46e5]/5 rounded-bl-full pointer-events-none" />

          <div className="relative z-10 space-y-4">
            <div className="w-16 h-16 mx-auto rounded-full bg-[#ede9fe] text-[#4f46e5] flex items-center justify-center">
              <Headphones className="w-8 h-8" />
            </div>

            <div className="space-y-1">
              <h2 className="font-extrabold text-base text-[#0f172a]">Still need help?</h2>
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center pt-2">
              <button
                ...
                className="bg-[#7c3aed] hover:bg-[#6d28d9] text-white rounded-full px-8 py-3.5 font-bold text-xs shadow-sm active:scale-95 transition-all flex items-center justify-center gap-2 cursor-pointer"
              >
              ...
              <button
                ...
                className="bg-white text-[#4f46e5] border border-[#cbd5e1] rounded-full px-8 py-3.5 font-bold text-xs hover:bg-[#f1f5f9] active:scale-95 transition-all flex items-center justify-center gap-2 cursor-pointer"
              >
      ...
      {activeModal === 'contact' && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl border border-[#cbd5e1] space-y-4 animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between pb-2 border-b border-[#cbd5e1]/30">
              <div className="flex items-center gap-2 text-[#4f46e5]">
                <Mail className="w-5 h-5" />
                <h3 className="text-sm font-extrabold text-[#0f172a]">Open Support Ticket</h3>
              </div>
              <button
                onClick={() => setActiveModal(null)}
                className="p-1.5 text-[#64748b] hover:text-[#0f172a] hover:bg-[#f1f5f9] rounded-full"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <form onSubmit={handleContactSubmit} className="space-y-3.5">
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-bold text-[#475569] uppercase tracking-wider">Communication Email</label>
                <input
                  type="email"
                  value={contactEmail}
                  onChange={(e) => setContactEmail(e.target.value)}
                  className="w-full p-3 rounded-xl border border-[#cbd5e1]/60 text-xs font-semibold focus:outline-none focus:ring-1 focus:ring-[#4f46e5] focus:border-[#4f46e5]"
              ...
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-bold text-[#475569] uppercase tracking-wider">Subject / Help Topic</label>
                <input
                  type="text"
              ...
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-bold text-[#475569] uppercase tracking-wider">Describe your issue in detail</label>
                <textarea
                  value={contactMessage}
                  onChange={(e) => setContactMessage(e.target.value)}
                  rows={4}
                  className="w-full p-3 rounded-xl border border-[#cbd5e1]/60 text-xs font-semibold focus:outline-none focus:ring-1 focus:ring-[#4f46e5] focus:border-[#4f46e5] resize-none"
              ...
              <div className="flex gap-3 pt-2">
                <button
                  type="button"
                  onClick={() => setActiveModal(null)}
                  className="flex-1 py-3 rounded-full border border-[#64748b] text-[#475569] font-bold text-xs hover:bg-[#f1f5f9] cursor-pointer"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="flex-1 py-3 bg-[#4f46e5] hover:bg-[#6d28d9] disabled:opacity-60 text-white font-bold text-xs rounded-full shadow-md flex items-center justify-center gap-1.5 transition-all cursor-pointer"
                >
          ...
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl border border-[#cbd5e1] space-y-4 animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between pb-2 border-b border-[#cbd5e1]/30">
              <div className="flex items-center gap-2 text-amber-600">
                ...
                <h3 className="text-sm font-extrabold text-[#0f172a]">Report Job Posting</h3>
              </div>
              <button
                onClick={() => setActiveModal(null)}
                className="p-1.5 text-[#64748b] hover:text-[#0f172a] hover:bg-[#f1f5f9] rounded-full"
            ...
            <form onSubmit={handleReportJobSubmit} className="space-y-3.5">
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-bold text-[#475569] uppercase tracking-wider">Find the job listing</label>
                {reportJobSelected ? (
                  ...
                  <>
                    <input
                      ...
                      className="w-full p-3 rounded-xl border border-[#cbd5e1]/60 text-xs font-semibold focus:outline-none focus:ring-1 focus:ring-amber-600 focus:border-amber-600"
                      placeholder="e.g., Balayage Hair Stylist or salon name"
                    />
                    {reportJobHits.length > 0 && (
                      <div className="max-h-36 overflow-y-auto rounded-xl border border-[#cbd5e1]/60 divide-y divide-[#cbd5e1]/30">
                        {reportJobHits.map((hit) => (
                          ...
                          >
                            <span className="block font-bold text-[#0f172a]">{hit.title}</span>
                            <span className="block text-[10px] text-[#475569]">{hit.salonName}{hit.city ? ` • ${hit.city}` : ''}{hit.state ? `, ${hit.state}` : ''}</span>
                          </button>
                        ))}
                      </div>
                    )}
              ...
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-bold text-[#475569] uppercase tracking-wider">Reason for reporting</label>
                <select
                  value={reportJobReason}
                  onChange={(e) => setReportJobReason(e.target.value)}
                  className="w-full p-3 rounded-xl border border-[#cbd5e1]/60 text-xs font-semibold focus:outline-none focus:ring-1 focus:ring-amber-600 focus:border-amber-600 bg-white"
                  required
                >
              ...
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-bold text-[#475569] uppercase tracking-wider">Additional Trust Information</label>
                <textarea
                  value={reportJobNotes}
                  onChange={(e) => setReportJobNotes(e.target.value)}
                  rows={3}
                  className="w-full p-3 rounded-xl border border-[#cbd5e1]/60 text-xs font-semibold focus:outline-none focus:ring-1 focus:ring-amber-600 focus:border-amber-600 resize-none"
          ...
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl border border-[#cbd5e1] space-y-4 animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between pb-2 border-b border-[#cbd5e1]/30">
              <div className="flex items-center gap-2 text-rose-600">
                ...
                <h3 className="text-sm font-extrabold text-[#0f172a]">Report Employer / Salon</h3>
              </div>
              <button
                onClick={() => setActiveModal(null)}
                className="p-1.5 text-[#64748b] hover:text-[#0f172a] hover:bg-[#f1f5f9] rounded-full"
            ...
            <form onSubmit={handleReportEmployerSubmit} className="space-y-3.5">
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-bold text-[#475569] uppercase tracking-wider">Find the salon / employer</label>
                {reportSalonSelected ? (
                  ...
                  <>
                    <input
                      ...
                      className="w-full p-3 rounded-xl border border-[#cbd5e1]/60 text-xs font-semibold focus:outline-none focus:ring-1 focus:ring-rose-600 focus:border-rose-600"
                    ...
                    {reportSalonHits.length > 0 && (
                      <div className="max-h-36 overflow-y-auto rounded-xl border border-[#cbd5e1]/60 divide-y divide-[#cbd5e1]/30">
                        {reportSalonHits.map((hit) => (
                          ...
                          >
                            <span className="block font-bold text-[#0f172a]">{hit.salonName}</span>
                            {(hit.city || hit.state) && (
                              <span className="block text-[10px] text-[#475569]">{[hit.city, hit.state].filter(Boolean).join(', ')}</span>
                            )}
                          </button>
                        ))}
                      </div>
              ...
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-bold text-[#475569] uppercase tracking-wider">Violations committed</label>
                <select
                  value={reportEmployerReason}
                  onChange={(e) => setReportEmployerReason(e.target.value)}
                  className="w-full p-3 rounded-xl border border-[#cbd5e1]/60 text-xs font-semibold focus:outline-none focus:ring-1 focus:ring-rose-600 focus:border-rose-600 bg-white"
                  required
                >
                  <option value="">-- Select reason --</option>
              ...
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-bold text-[#475569] uppercase tracking-wider">Detailed Incident Description</label>
                <textarea
                  value={reportEmployerNotes}
                  onChange={(e) => setReportEmployerNotes(e.target.value)}
                  rows={3}
                  className="w-full p-3 rounded-xl border border-[#cbd5e1]/60 text-xs font-semibold focus:outline-none focus:ring-1 focus:ring-rose-600 focus:border-rose-600 resize-none"
      ...
      {activeModal === 'live_chat' && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full h-[520px] flex flex-col shadow-2xl border border-[#cbd5e1] overflow-hidden animate-in zoom-in-95 duration-200">

            {/* Chat header */}
            <div className="bg-gradient-to-r from-[#4f46e5] to-[#7c3aed] text-white p-4 flex items-center justify-between">
            ...
            <div className="flex-1 p-4 overflow-y-auto space-y-3 bg-[#f8fafc] text-xs">
                ...
                >
                  <div className={`max-w-[85%] rounded-2xl p-3.5 space-y-1 ${
                    msg.sender === 'user'
                      ...
                      : 'bg-white text-[#0f172a] border border-[#cbd5e1]/40 rounded-bl-none shadow-xs'
                  }`}>
              ...
              {isTyping && (
                <div className="flex justify-start">
                  <div className="bg-white border border-[#cbd5e1]/40 rounded-2xl p-3.5 rounded-bl-none shadow-xs text-[#64748b] italic text-[11px] flex items-center gap-1.5">
                    <span className="w-1 h-1 bg-[#4f46e5] rounded-full animate-bounce" />
                    <span className="w-1 h-1 bg-[#4f46e5] rounded-full animate-bounce [animation-delay:0.2s]" />
                    <span className="w-1 h-1 bg-[#4f46e5] rounded-full animate-bounce [animation-delay:0.4s]" />
            ...
            <div className="p-2 border-t border-[#cbd5e1]/30 bg-[#f1f5f9]/40 flex gap-1.5 overflow-x-auto whitespace-nowrap scrollbar-none">
              {quickChatPrompts.map((p, idx) => (
                <button
                  key={idx}
                  onClick={() => handleSendMessage(p)}
                  className="bg-white border border-[#cbd5e1] rounded-full px-3 py-1.5 text-[10px] text-[#4f46e5] font-bold hover:bg-[#ede9fe]/20 shrink-0 transition-colors cursor-pointer"
                >
                  {p}
                </button>
              ))}
            ...
            <div className="p-3 border-t border-[#cbd5e1]/30 bg-white flex items-center gap-2">
              <input
                type="text"
                ...
                className="flex-1 bg-[#f1f5f9] py-2.5 px-4 rounded-xl border border-transparent focus:bg-white focus:border-[#4f46e5] outline-none text-xs transition-all"
              />
              <button
                onClick={() => handleSendMessage()}
                disabled={!chatInput.trim()}
                className="w-10 h-10 rounded-full bg-[#4f46e5] disabled:bg-[#64748b]/40 text-white flex items-center justify-center shrink-0 hover:bg-[#6d28d9] transition-colors cursor-pointer"
              >
                <Send className="w-4 h-4" />
              </button>
            </div>
      ...
      <nav className="fixed bottom-0 left-0 w-full flex justify-around items-center py-3 bg-white border-t border-[#cbd5e1]/40 z-30 sm:hidden shadow-[0_-4px_12px_rgba(15,23,42,0.05)]">
        <button
          onClick={() => {
            onBack();
        ...
        >
          ...
          <span className="text-[10px] font-bold mt-1 text-[#64748b]">Jobs</span>
        </button>
        <button
          onClick={() => {
            onBack();
        ...
        >
          ...
          <span className="text-[10px] font-bold mt-1 text-[#64748b]">Apps</span>
        </button>
        <button
          onClick={() => {
            onBack();
        ...
        >
          ...
          <span className="text-[10px] font-bold mt-1 text-[#64748b]">Saved</span>
        </button>
        <button
          className="flex flex-col items-center justify-center bg-[#ede9fe] text-[#4f46e5] rounded-full px-4 py-1.5 active:scale-90 transition-transform cursor-pointer"
        >
          <Headphones className="w-5 h-5 text-[#4f46e5]" />
          <span className="text-[10px] font-bold mt-0.5 text-[#4f46e5]">Support</span>
        </button>
        <button
          onClick={() => {
            onBack();
        ...
        >
          ...
          <span className="text-[10px] font-bold mt-1 text-[#64748b]">Profile</span>
        </button>
      </nav>
    </div>
  );
