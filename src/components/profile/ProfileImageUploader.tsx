...
export const ProfileImageUploader: React.FC<ProfileImageUploaderProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl max-w-lg w-full overflow-hidden shadow-2xl border border-[#cbd5e1]/60 flex flex-col max-h-[90vh]">
        {/* Modal Header */}
        <div className="p-5 bg-gradient-to-r from-[#4f46e5] to-[#6d28d9] text-white flex items-center justify-between">
        ...
        {/* Navigation Tabs */}
        <div className="flex border-b border-[#cbd5e1]/40 bg-[#f8fafc] p-1.5 gap-1">
          <button
            ...
            className={`flex-1 py-2.5 rounded-2xl text-xs font-bold transition-all flex items-center justify-center gap-1.5 cursor-pointer ${
              activeTab === 'camera'
                ? 'bg-white text-[#4f46e5] shadow-xs border border-[#cbd5e1]/50'
        ...
        <div className="p-6 overflow-y-auto flex-1 flex flex-col items-center justify-center">
            ...
            <div className="w-full flex flex-col items-center gap-4">
              {capturedPhoto ? (
                ...
                <div className="flex flex-col items-center gap-4 w-full">
                  <div className="relative w-56 h-56 rounded-full overflow-hidden border-4 border-[#7c3aed] shadow-lg bg-black">
                    <img
                      src={capturedPhoto}
                  ...
                  <div className="flex gap-3">
                    <button
                      onClick={startCamera}
                      className="px-4 py-2 rounded-full border border-[#cbd5e1] bg-white text-[#4f46e5] hover:bg-[#ede9fe]/50 text-xs font-bold flex items-center gap-1.5 transition-colors cursor-pointer"
                    >
                ...
                <div className="p-6 rounded-2xl bg-rose-50 border border-rose-200 text-center max-w-sm space-y-3">
                  ...
                  <button
                    onClick={() => setActiveTab('upload')}
                    className="px-4 py-2 rounded-full bg-[#4f46e5] text-white text-xs font-bold hover:bg-[#6d28d9] transition-colors cursor-pointer"
                  >
                ...
                <div className="flex flex-col items-center gap-4 w-full">
                  <div className="relative w-60 h-60 sm:w-64 sm:h-64 rounded-full overflow-hidden border-4 border-[#4f46e5] shadow-xl bg-slate-900 flex items-center justify-center group">
                    <video
                      ref={videoRef}
                      autoPlay
                      playsInline
                  ...
                  {/* Camera Control Bar */}
                  <div className="flex items-center gap-4 bg-[#f1f5f9] px-4 py-2 rounded-full border border-[#cbd5e1]/60 text-xs text-[#475569]">
                    <button
                      ...
                      className={`p-2 rounded-full transition-colors cursor-pointer flex items-center gap-1 ${
                        ringLightEffect ? 'bg-amber-100 text-amber-800 font-bold' : 'hover:bg-white text-[#64748b]'
                      }`}
                    ...
                    <button
                      ...
                      className={`p-2 rounded-full transition-colors cursor-pointer flex items-center gap-1 ${
                        isMirrored ? 'bg-[#ede9fe] text-[#4f46e5] font-bold' : 'hover:bg-white text-[#64748b]'
                      }`}
                  ...
                  <button
                    ...
                    className="mt-2 px-8 py-3 bg-[#7c3aed] hover:bg-[#6d28d9] text-white text-sm font-extrabold rounded-full shadow-lg hover:shadow-xl active:scale-95 transition-all cursor-pointer flex items-center gap-2"
                  >
          ...
          {activeTab === 'upload' && (
            <div className="w-full flex flex-col items-center gap-5">
              <div
                ...
                className="w-full border-2 border-dashed border-[#cbd5e1] hover:border-[#4f46e5] rounded-3xl p-8 text-center bg-[#f8fafc] hover:bg-[#ede9fe]/20 transition-all cursor-pointer flex flex-col items-center gap-3"
              >
                <div className="w-16 h-16 rounded-full bg-[#ede9fe] text-[#4f46e5] flex items-center justify-center shadow-xs">
                  <Upload className="w-8 h-8" />
                </div>
                <div>
                  <h3 className="text-sm font-bold text-[#0f172a]">Click to upload profile photo</h3>
                  <p className="text-xs text-[#475569] mt-1">Supports JPG, PNG, WEBP (Max 10MB)</p>
                </div>
                <button
                  type="button"
                  className="px-4 py-2 bg-[#4f46e5] text-white text-xs font-bold rounded-full shadow-sm hover:bg-[#6d28d9] transition-colors"
                >
                  Select File
                </button>
              </div>
              ...
              {selectedImage && activeTab === 'upload' && (
                <div className="flex flex-col items-center gap-2">
                  <span className="text-xs font-semibold text-[#4f46e5]">Selected Image Preview:</span>
                  <div className="w-32 h-32 rounded-full overflow-hidden border-2 border-[#4f46e5] shadow-md">
          ...
          {activeTab === 'presets' && (
            <div className="w-full space-y-3">
              <p className="text-xs font-semibold text-[#475569] text-center">
              ...
              <div className="grid grid-cols-3 gap-3">
                {PRESET_HEADSHOTS.map((preset) => (
                  <button
                    ...
                    className={`relative rounded-2xl overflow-hidden border-2 transition-all cursor-pointer group aspect-square ${
                      selectedImage === preset.url
                        ? 'border-[#7c3aed] ring-2 ring-[#7c3aed]/40 shadow-md scale-102'
                  ...
                  >
                    ...
                    {selectedImage === preset.url && (
                      <div className="absolute top-1.5 right-1.5 w-5 h-5 rounded-full bg-[#7c3aed] text-white flex items-center justify-center">
        ...
        <div className="p-4 bg-[#f8fafc] border-t border-[#cbd5e1]/40 flex items-center justify-between gap-3">
          ...
          <div className="flex flex-col items-end gap-2">
            ...
            <div className="flex gap-2.5">
              <button
                onClick={onClose}
                className="px-5 py-2.5 rounded-full border border-[#cbd5e1] bg-white text-[#475569] hover:bg-gray-50 text-xs font-bold transition-colors cursor-pointer"
              >
                Cancel
              </button>
              <button
                ...
                className="px-6 py-2.5 rounded-full bg-[#7c3aed] hover:bg-[#6d28d9] disabled:opacity-50 text-white text-xs font-extrabold shadow-md transition-all cursor-pointer flex items-center gap-1.5"
              >
