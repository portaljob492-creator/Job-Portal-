...
Add local and production callback URLs to Redirect URLs.
The OTP screen expects a six-digit signup token. In Authentication → Email Templates → Confirm signup, include {{ .Token }} in the message instead of relying only on {{ .ConfirmationURL }}.
To use Google or Apple buttons, enable those providers under Authentication → Providers and supply their provider credentials.
Password recovery redirects to /?recovery=1; add that origin to the allowed redirect URLs.
Database overview
Table	Purpose
