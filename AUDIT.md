...
| P1-11 | **Vercel SPA rewrites cover only `/app/jobs*`.** `vercel.json` rewrites just 2 sources, but `routing.ts` defines ~20 routes (`/login`, `/dashboard/*`, `/messages`, `/admin/*`…). Standalone root deploys 404 on refresh/deep-link everywhere else. Works only if `VITE_APP_BASE_PATH=/app/jobs` — an undocumented coupling. | `vercel.json` vs `routing.ts` | ✅ **Fixed:** catch-all rewrite `/(.*) → /index.html` in `vercel.json` (filesystem still wins for static assets); regression-guarded by `npm run test:vercel` (`scripts/test-vercel-spa-routes.mjs`). |
